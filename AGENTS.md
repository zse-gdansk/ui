...
4. **Ruch według `docs/motion.md`, wygląd według `docs/design.md`**: konkretne wartości, nie „coś podobnego”. Hover tylko w `@media (hover: hover)` i bez żadnych przejść koloru, stan nie zmienia grubości fontu, bez ręcznego `letter-spacing`, `prefers-reduced-motion` w każdym komponencie z ruchem.
