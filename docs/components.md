...
| `Sidebar`, `SidebarHeader`, `SidebarContent`, `SidebarGroup`, `SidebarItem`, `SidebarSub`, `SidebarBack`, `SidebarFooter`, `isActivePath` | Collapsible, Menu, Tooltip | ikony w stałej kolumnie i etykiety ucinane krawędzią, więc zwijanie nic nie przesuwa; po zwinięciu najechanie zamienia znak w przycisk rozwijania, tooltipy (kolejne od razu), licznik jako kropka, grupy z kreską w tym samym wierszu; `SidebarSub` rozwija się w miejscu, a po zwinięciu otwiera menu obok ikony; `render` dla linków routera; `SidebarContent` z `view` i `level` jako nawigacja podstron (np. ustawienia): głębiej wjeżdża z prawej, płycej z lewej, stary widok wyjeżdża równocześnie, `SidebarBack` na powrót; `opensView` na pozycji wchodzącej w taki widok (i zawsze na `SidebarBack`) zostawia wysuwany panel otwarty na telefonie |
