...
| `AppHeader`, `HeaderBreadcrumbs`, `HeaderAction`                                                                                          | – (Breadcrumbs, Tooltip)   | pasek dla `header`: przycisk panelu na telefonie, breadcrumbs albo `title`, akcje zawsze w całości; breadcrumbs deklaruje strona przez `<HeaderBreadcrumbs>` (portal do paska, środek chowa się do „…”); `HeaderAction` 32×32 z tooltipem, `shortcut` działa globalnie i jest w tooltipie, `badge` jako kropka, `render` dla linków, `priority="secondary"` chowa akcję poniżej 640px do menu „⋯” (skrót dalej działa, kropka przechodzi na „⋯”)                                                                                                                                                                                                                                                                        |
...
| `PageTabs`, `PageTab`                                                                                                                     | – (ScrollArea)             | zakładki strony jako linki do podstron (`nav` z `aria-current`, `render` dla routera), aktywna z adresu; kreska w kolorze akcentu przesuwa się do aktywnej (pierwsze ustawienie bez przejścia), aktywna bez zmiany grubości; na wąskim ekranie przewijanie w poziomie z wygaszonymi krawędziami i aktywną w widoku; pod `PageHeader` bliżej; `badge`                                                                                                                                                                                                                                                                                                                                                                    |
| `Sidebar`, `SidebarHeader`, `SidebarContent`, `SidebarGroup`, `SidebarItem`, `SidebarSub`, `SidebarBack`, `SidebarFooter`, `isActivePath` | Collapsible, Menu, Tooltip | ikony w stałej kolumnie i etykiety ucinane krawędzią, więc zwijanie nic nie przesuwa; po zwinięciu najechanie zamienia znak w przycisk rozwijania, tooltipy (kolejne od razu), licznik jako kropka, grupy z kreską w tym samym wierszu; `SidebarSub` rozwija się w miejscu, a po zwinięciu otwiera menu obok ikony; `render` dla linków routera; `SidebarContent` z `view` i `level` jako nawigacja podstron (np. ustawienia): głębiej wjeżdża z prawej, płycej z lewej, stary widok wyjeżdża równocześnie, `SidebarBack` na powrót; `opensView` na pozycji wchodzącej w taki widok (przy najechaniu strzałka „›”, po zwinięciu zamiast ikony) (i zawsze na `SidebarBack`) zostawia wysuwany panel otwarty na telefonie |
...
| `AppLoader`                                                                                                                               | – (Spinner)                | pełnoekranowy ekran wczytywania aplikacji (sesja, konto, uprawnienia): widoczny od pierwszej klatki, także w HTML z serwera, bez wejścia; aplikacja renderuje się pod spodem jako `inert` i po wczytaniu wyłania się przenikaniem (200 ms); po 8 s „To trwa dłużej niż zwykle”, potem co 4 s losowy z kilku tekstów (przenikanie z ruchem, bez powtórki z rzędu); `error` z `onRetry`                                                                                                                                                                                                                                                                                                                                   |
...
Pasek i breadcrumbs strony. `AppHeader` idzie do layoutu, breadcrumbs do strony, bo dopiero ona zna nazwy (np. ucznia z bazy). Na pierwszym renderze na serwerze widać `title`, breadcrumbs pojawiają się po hydracji w tym samym miejscu.
...
Szerokość treści ustala strona, nie szkielet: `<main>` ma całą szerokość obok panelu, także tę zwolnioną po zwinięciu. Strony z tabelami i danymi nie ograniczają szerokości; formularze i dłuższy tekst kładą treść w `Container`, żeby linie nie ciągnęły się przez cały ekran.
...
| `Menu`, `MenuItem`, `MenuSub`… | Menu                | podmenu z bezpiecznym trójkątem (na dotyku otwierane i zamykane stuknięciem), działające skróty (`mod+d`), checkbox i radio, `render` dla linków routera, `variant` `danger` (stale czerwone, Usuń) i `danger-hover` (czerwone przy najechaniu, Wyloguj), `MenuSub` z `suffix` na bieżącą wartość |
...
| `Spinner`, `Kbd`, `Separator`                     | Separator              | `Spinner` (i `Spokes`): osiem kresek jak w iOS, jasność biegnie po okręgu, ten sam wskaźnik w `Button`, toaście, alercie, comboboxie, `InputGroup` i `CodeField`; `Kbd` z systemowymi skrótami                                                                                                                                                         |
